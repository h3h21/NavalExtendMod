...
namespace skpCustomModule
{
	...
	public class SkyBoxChanger : SingleInstance<SkyBoxChanger>
	{
		...
		private void Update()
		{
			...
			else
			{
				if (!this.Init)
				{
					...
					if (platform != (RuntimePlatform)1)
					{
						if (platform != (RuntimePlatform)2)
						{
							if (platform != (RuntimePlatform)13)
							{
								this.modAssetBundle = ModResource.GetAssetBundle("skyasset");
								this.WaterAsset = ModResource.GetAssetBundle("waterasset");
							}
							else
							{
								this.modAssetBundle = ModResource.GetAssetBundle("skyassetMac");
								this.WaterAsset = ModResource.GetAssetBundle("waterassetMac");
							}
						}
						else
						{
					...
					for (int i = 0; i < 10; i++)
					{
						if (!this.SkyMaterialContainer.ContainsKey(i))
						{
							this.SkyMaterialContainer.Add(i, this.modAssetBundle.LoadAsset<Material>(this.SkyNameContainer[i]));
						}
						else
						{
							this.SkyMaterialContainer[i] = this.modAssetBundle.LoadAsset<Material>(this.SkyNameContainer[i]);
						}
						if (!this.SkyFogContainer.ContainsKey(i))
						{
							this.SkyFogContainer.Add(i, this.modAssetBundle.LoadAsset<Cubemap>(this.SkyNameContainer[i] + "Fog"));
						}
						else
						{
							this.SkyFogContainer[i] = this.modAssetBundle.LoadAsset<Cubemap>(this.SkyNameContainer[i] + "Fog");
						}
					}
					this.WaterObject = this.WaterAsset.LoadAsset<GameObject>("WaterObject.prefab");
					this.InsideWater = this.WaterAsset.LoadAsset<Material>("InsideWater.mat");
				}
				if (this.CameraComponent == null)
				{
					this.CameraComponent = Camera.main;
					this.FogComponent = Camera.main.gameObject.GetComponents<ColorfulFog>();
				}
				if (this.LightObject == null)
				{
					if (StatMaster.isMP)
					{
						...
						this.FogComponentNum = 0;
						this.LightComponent = this.LightObject.GetComponent<Light>();
						this.FogValue[0] = this.CameraComponent.farClipPlane.ToString();
						this.FogValue[1] = this.FogComponent[this.FogComponentNum].fogDensity.ToString();
						this.FogValue[2] = this.FogComponent[this.FogComponentNum].startDistance.ToString();
						this.FogValue[3] = this.FogComponent[this.FogComponentNum].heightDensity.ToString();
						this.FogValue[4] = this.FogComponent[this.FogComponentNum].height.ToString();
					}
			...
			if (this.changeflag || AdCustomModuleMod.skyboxchangeFlag)
			{
				if (!StatMaster.isMP)
				{
			...
			if (this.toggle_skyboxApply || this.networkflag)
			{
				...
				this.CameraComponent.farClipPlane = float.Parse(this.FogValue[0]);
				...
				this.SkyMeshFilter = this.SkyObject.GetComponent<MeshFilter>();
				ModMesh mesh = ModResource.GetMesh("skyMesh");
				this.SkyMeshFilter.mesh = mesh;
				this.FogRenderer = this.SkyObject.GetComponent<MeshRenderer>();
				this.FogRenderer.material = this.SkyMaterialContainer[this.selector];
				this.LightComponent = this.LightObject.GetComponent<Light>();
				Color color = default(Color);
				color.r = float.Parse(this.SkyColorValue[0]);
				color.g = float.Parse(this.SkyColorValue[1]);
				color.b = float.Parse(this.SkyColorValue[2]);
				...
				if (StatMaster.isMP && StatMaster.isHosting)
				{
					...
					{
						this.selector,
						float.Parse(this.FogValue[0]),
						float.Parse(this.FogValue[1]),
						float.Parse(this.FogValue[2]),
						float.Parse(this.FogValue[3]),
						float.Parse(this.FogValue[4]),
						float.Parse(this.SkyColorValue[0]),
						float.Parse(this.SkyColorValue[1]),
						float.Parse(this.SkyColorValue[2]),
			...
			if (this.toggle_waterApply || this.networkwaterEnable)
			{
				...
				if (!gameObject.transform.FindChild("WaterPlane00"))
				{
					...
					for (int num8 = 0; num8 < 3; num8++)
					{
						for (int num9 = 0; num9 < 3; num9++)
						{
							...
							Water water = gameObject2.GetComponent<Water>();
							if (water == null)
							{
								water = gameObject2.AddComponent<Water>();
							}
						}
					}
				}
				...
				this.WaterBouncyEnable = this.WaterBouncyEnableSwitch;
				this.WaterHeight = float.Parse(this.WaterValue[3]);
				for (int num18 = 0; num18 < 3; num18++)
				{
					for (int num19 = 0; num19 < 3; num19++)
					{
						Water component = this.WaterObjectArray[num18, num19].transform.FindChild("WaterProDaytime").gameObject.transform.GetComponent<Water>();
					}
				}
				...
				if (StatMaster.isMP && StatMaster.isHosting)
				{
					ModNetworking.SendToAll(AdCustomModuleMod.msgWaterData.CreateMessage(new object[]
					{
						this.WaterEnable,
						this.WaterBouncyEnable,
						float.Parse(this.WaterValue[0]),
						float.Parse(this.WaterValue[1]),
						float.Parse(this.WaterValue[2]),
						float.Parse(this.WaterValue[3]),
						float.Parse(this.WaterValue[4]),
						float.Parse(this.WaterValue[5]),
						float.Parse(this.WaterValue[6]),
						float.Parse(this.WaterValue[7]),
						float.Parse(this.WaterValue[8]),
						float.Parse(this.WaterValue[9]),
						float.Parse(this.WaterValue[10]),
						float.Parse(this.WaterValue[11]),
						float.Parse(this.WaterValue[12]),
						float.Parse(this.WaterValue[13]),
						float.Parse(this.WaterFogValue[0]),
						float.Parse(this.WaterFogValue[1]),
						float.Parse(this.WaterFogValue[2]),
						float.Parse(this.WaterRefrColor[0]),
						float.Parse(this.WaterRefrColor[1]),
						float.Parse(this.WaterRefrColor[2]),
						float.Parse(this.WaterRefrColor[3]),
						float.Parse(this.WaterFadeColor[0]),
						float.Parse(this.WaterFadeColor[1]),
						float.Parse(this.WaterFadeColor[2]),
						float.Parse(this.WaterFadeColor[3]),
						float.Parse(this.WaterFogColor[0]),
						float.Parse(this.WaterFogColor[1]),
						float.Parse(this.WaterFogColor[2]),
						float.Parse(this.WaterFogColor[3]),
						float.Parse(this.InWaterFarColor[0]),
						float.Parse(this.InWaterFarColor[1]),
						float.Parse(this.InWaterFarColor[2]),
						float.Parse(this.InWaterFarColor[3]),
						float.Parse(this.InWaterRefColor[0]),
						float.Parse(this.InWaterRefColor[1]),
						float.Parse(this.InWaterRefColor[2]),
						float.Parse(this.InWaterRefColor[3])
					}));
				}
			}
			if (this.WaterEnable)
			{
				float num20 = this.CameraComponent.transform.position.x / 4000f;
				double value = (double)(this.CameraComponent.transform.position.z / 4000f);
				float num21 = (float)Math.Round((double)num20, MidpointRounding.AwayFromZero);
				float num22 = (float)Math.Round(value, MidpointRounding.AwayFromZero);
				for (int num23 = 0; num23 < 3; num23++)
				{
					for (int num24 = 0; num24 < 3; num24++)
					{
						float num25 = -4000f + 4000f * (float)num23 + 4000f * num21;
						float num26 = float.Parse(this.WaterValue[3]);
						float num27 = -4000f + 4000f * (float)num24 + 4000f * num22;
						this.WaterObjectArray[num23, num24].transform.position = new Vector3(num25, num26, num27);
					}
				}
			}
		...
		private void TerainObjectChecker()
		{
			if (StatMaster.isMP)
			{
				...
				case SkyBoxChanger.TerainType.ipsilon:
					this.FogComponent[2].enabled = false;
					this.FogComponent[1].enabled = false;
					this.FogComponent[0].enabled = true;
				...
				this.FogComponentNum = 0;
				this.FogComponent[this.FogComponentNum].fogDensity = float.Parse(this.FogValue[1]);
				this.FogComponent[this.FogComponentNum].startDistance = float.Parse(this.FogValue[2]);
				this.FogComponent[this.FogComponentNum].heightDensity = float.Parse(this.FogValue[3]);
				this.FogComponent[this.FogComponentNum].height = float.Parse(this.FogValue[4]);
				this.FogComponent[this.FogComponentNum].fogCube = this.SkyFogContainer[this.selector];
			...
			else if (!StatMaster.isClient)
			{
				...
				{
					this.FogComponent[1].enabled = true;
					this.FogComponent[0].enabled = false;
					if (this.SkyObject == null)
					{
						this.SkyObject = new GameObject();
						this.SkyObject.name = "SKY SPHERE";
		...
		private void OnGUI()
		{
			this.gui_ACMOptions = delegate(int p0)
			{
				if (this.Init)
				{
					...
					this.ToggleIndent("SkyBoxSelector", 10f, ref this.indentflag, delegate
					{
						GUILayout.BeginVertical("box", new GUILayoutOption[0]);
						if (GUILayout.Button("Sunset", new GUILayoutOption[0]))
						{
							this.selector = 0;
						...
						if (GUILayout.Button("Space", new GUILayoutOption[0]))
						{
							this.selector = 4;
							this.changeflag = true;
						}
						if (GUILayout.Button("Sunrise", new GUILayoutOption[0]))
						{
							this.selector = 5;
							this.changeflag = true;
						}
						if (GUILayout.Button("DayLight", new GUILayoutOption[0]))
						{
							this.selector = 6;
							this.changeflag = true;
						}
						if (GUILayout.Button("Twilight", new GUILayoutOption[0]))
						{
							this.selector = 7;
							this.changeflag = true;
						}
						if (GUILayout.Button("MidNight", new GUILayoutOption[0]))
						{
							this.selector = 8;
							this.changeflag = true;
						}
						if (GUILayout.Button("GreenBack", new GUILayoutOption[0]))
						{
							this.selector = 9;
							this.changeflag = true;
						}
					...
					this.ToggleIndent("AdvancedSettings", 10f, ref this.indentflag2, delegate
					{
						GUILayout.BeginVertical("box", new GUILayoutOption[0]);
						GUILayout.BeginHorizontal(new GUILayoutOption[0]);
						GUILayout.Label("RenderDistance : " + string.Format("{0,8:F1}", this.CameraComponent.farClipPlane), new GUILayoutOption[0]);
						GUILayout.FlexibleSpace();
						this.ClampTextField("", ref this.FogValue[0], ref this.FogValue_temp[0], 0f, 0f, "RenderDistance", 10, false);
						GUILayout.EndHorizontal();
						GUILayout.BeginHorizontal(new GUILayoutOption[0]);
						GUILayout.Label("FogDensity : " + string.Format("{0,8:F5}", this.FogComponent[this.FogComponentNum].fogDensity), new GUILayoutOption[0]);
						GUILayout.FlexibleSpace();
						this.FogValue[1] = GUILayout.TextField(this.FogValue[1], new GUILayoutOption[] { GUILayout.Width(85f) });
						GUILayout.EndHorizontal();
						GUILayout.BeginHorizontal(new GUILayoutOption[0]);
						GUILayout.Label("StatDistance : " + string.Format("{0,8:F1}", this.FogComponent[this.FogComponentNum].startDistance), new GUILayoutOption[0]);
						GUILayout.FlexibleSpace();
						this.FogValue[2] = GUILayout.TextField(this.FogValue[2], new GUILayoutOption[] { GUILayout.Width(85f) });
						GUILayout.EndHorizontal();
						GUILayout.BeginHorizontal(new GUILayoutOption[0]);
						GUILayout.Label("HeightDensity : " + string.Format("{0,8:F1}", this.FogComponent[this.FogComponentNum].heightDensity), new GUILayoutOption[0]);
						GUILayout.FlexibleSpace();
						this.FogValue[3] = GUILayout.TextField(this.FogValue[3], new GUILayoutOption[] { GUILayout.Width(85f) });
						GUILayout.EndHorizontal();
						GUILayout.BeginHorizontal(new GUILayoutOption[0]);
						GUILayout.Label("Height : " + string.Format("{0,8:F1}", this.FogComponent[this.FogComponentNum].height), new GUILayoutOption[0]);
						GUILayout.FlexibleSpace();
						this.FogValue[4] = GUILayout.TextField(this.FogValue[4], new GUILayoutOption[] { GUILayout.Width(85f) });
						GUILayout.EndHorizontal();
						GUILayout.Label("LightColor", new GUILayoutOption[0]);
						GUILayout.BeginHorizontal(new GUILayoutOption[0]);
						GUILayout.Label("R : " + string.Format("{0,8:F5}", this.LightComponent.color.r), new GUILayoutOption[0]);
						GUILayout.Label("G : " + string.Format("{0,8:F5}", this.LightComponent.color.g), new GUILayoutOption[0]);
						GUILayout.Label("B : " + string.Format("{0,8:F5}", this.LightComponent.color.b), new GUILayoutOption[0]);
						GUILayout.EndHorizontal();
						this.SliderandTextField("R :", ref this.SkyColorValue[0], ref this.SkyColorValue_temp[0], 0f, 1f, "Svalue_R", 7, true, 150f);
						this.SliderandTextField("G :", ref this.SkyColorValue[1], ref this.SkyColorValue_temp[1], 0f, 1f, "Svalue_G", 7, true, 150f);
						this.SliderandTextField("B :", ref this.SkyColorValue[2], ref this.SkyColorValue_temp[2], 0f, 1f, "Svalue_B", 7, true, 150f);
						GUILayout.BeginHorizontal(new GUILayoutOption[0]);
						...
						this.SkyColorTex.SetPixel(0, 0, new Color(this.LightComponent.color.r, this.LightComponent.color.g, this.LightComponent.color.b));
						this.SkyColorTex.Apply();
						this.SkyColorGUI.normal.background = this.SkyColorTex;
						this.SkyColorGUI.fixedHeight = 15f;
						this.SkyColorGUI.fixedWidth = 100f;
						GUILayout.BeginHorizontal(this.SkyColorGUI, new GUILayoutOption[]
						{
							GUILayout.Height(15f),
						...
						this.SkyColorTex.SetPixel(0, 0, new Color(float.Parse(this.SkyColorValue[0]), float.Parse(this.SkyColorValue[1]), float.Parse(this.SkyColorValue[2])));
						this.SkyColorTex.Apply();
						this.SkyColorGUI.normal.background = this.SkyColorTex;
						this.SkyColorGUI.fixedHeight = 15f;
						this.SkyColorGUI.fixedWidth = 100f;
			...
			{
				this.windowHeight = 440f;
				if (this.indentflag)
				{
					this.windowHeight += 260f;
				}
				if (this.indentflag2)
				{
					this.windowHeight += 275f;
				}
				if (this.indentflag3)
				{
					this.windowHeight += 395f;
				}
				if (this.indentflag4)
				{
					this.windowHeight += 615f;
				}
				if (this.debug)
				{
					this.windowHeight += 500f;
				}
				this.windowRect.height = this.windowHeight;
				this.windowRect = GUI.Window(this.window_id, this.windowRect, this.gui_ACMOptions, "ACM Options");
		...
		{
			this.SkyMaterialContainer = new Dictionary<int, Material>();
			this.SkyFogContainer = new Dictionary<int, Cubemap>();
			this.SkyNameContainer = new Dictionary<int, string>
			{
				{ 0, "Sunset" },
				{ 1, "BlueSky" },
				{ 2, "Cloudy" },
		...
		public Dictionary<int, Material> SkyMaterialContainer;

		// Token: 0x04000555 RID: 1365
		public Dictionary<int, Cubemap> SkyFogContainer;

		// Token: 0x04000556 RID: 1366
		public Dictionary<int, string> SkyNameContainer;
		...
		public ColorfulFog[] FogComponent;

		// Token: 0x0400055D RID: 1373
		public int FogComponentNum;
		...
		public ModAssetBundle SkyAsset;
		...
		public ModAssetBundle WaterAsset;

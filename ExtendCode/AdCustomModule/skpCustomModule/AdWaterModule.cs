		...
		private void Update()
		{
			...
			if (!flag)
			{
				...
				if (flag2)
				{
					...
					GameObject gameObject = AdCustomModuleMod.mod4.WaterAsset.LoadAsset<GameObject>("WaterSplashPrefab");
					this.ProjectileWaterEffect = Object.Instantiate<GameObject>(gameObject);
					this.ProjectileWaterEffect.SetActive(false);
					this.ProjectileWaterEffect.transform.SetParent(AdCustomModuleMod.mod2.PMcomponent.transform);
					this.ProjectileWaterEffect.AddComponent<RandomSeed>();
					for (int i = 0; i < this.MAX_Effect_POOL_SIZE + 1; i++)
					{
						Transform transform = Object.Instantiate<GameObject>(this.ProjectileWaterEffect).transform;
						transform.name = "WaterSplashPrefab";
						transform.SetParent(AdCustomModuleMod.mod2.PMEffectPool.transform);
						this.ProjectileWaterEffectList.Add(transform);
						transform.gameObject.SetActive(false);
					}
					GameObject gameObject2 = AdCustomModuleMod.mod4.WaterAsset.LoadAsset<GameObject>("WaterMoveSplashPrefab");
					this.BlockWaterSplashEffect = Object.Instantiate<GameObject>(gameObject2.transform.GetComponentInChildren<ParticleSystem>().gameObject);
					this.BlockWaterSplashEffect.SetActive(false);
					this.BlockWaterSplashEffect.transform.SetParent(AdCustomModuleMod.mod2.PMcomponent.transform);
					this.BlockWaterSplashEffect.AddComponent<RandomSeed2>();
					GameObject gameObject3 = AdCustomModuleMod.mod4.WaterAsset.LoadAsset<GameObject>("WaterMoveSplashHPrefab");
					this.BlockWaterWaveEffect = Object.Instantiate<GameObject>(gameObject3.transform.GetComponentInChildren<ParticleSystem>().gameObject);
					this.BlockWaterWaveEffect.SetActive(false);
					this.BlockWaterWaveEffect.transform.SetParent(AdCustomModuleMod.mod2.PMcomponent.transform);
					this.BlockWaterWaveEffect.AddComponent<RandomSeed2>();
				}
			}
		}

...
namespace skpCustomModule
{
	...
	public class AdCustomModuleMod : ModEntryPoint
	{
		...
		public override void OnLoad()
		{
			...
			callbacksWrapper3[messageType2] += delegate(Message message)
			{
				if (StatMaster.isClient)
				{
					...
					AdCustomModuleMod.skyboxinfo[0] = (float)message.GetData(1);
					AdCustomModuleMod.skyboxinfo[1] = (float)message.GetData(2);
					AdCustomModuleMod.skyboxinfo[2] = (float)message.GetData(3);
					AdCustomModuleMod.skyboxinfo[3] = (float)message.GetData(4);
					AdCustomModuleMod.skyboxinfo[4] = (float)message.GetData(5);
					AdCustomModuleMod.skyboxinfo2[0] = (float)message.GetData(6);
					AdCustomModuleMod.skyboxinfo2[1] = (float)message.GetData(7);
					AdCustomModuleMod.skyboxinfo2[2] = (float)message.GetData(8);
					AdCustomModuleMod.FloorDeactive = (bool)message.GetData(9);
					AdCustomModuleMod.ExpandFloor = (bool)message.GetData(10);
					AdCustomModuleMod.ExExpandFloor = (bool)message.GetData(11);
					AdCustomModuleMod.scale = (float)message.GetData(12);
		...
		public override void OnEntityPrefabCreation(int entityId, GameObject prefab)
		{
			if (entityId == 1)
			{
				prefab.AddComponent<AdLevelBlockBehaviour>();
			}
		}
